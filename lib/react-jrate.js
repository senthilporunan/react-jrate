(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["react-jrate"] = factory();
	else
		root["react-jrate"] = factory();
})(this, function() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _libConstantsRatingConstantsJs = __webpack_require__(158);

	var _libConstantsRatingConstantsJs2 = _interopRequireDefault(_libConstantsRatingConstantsJs);

	var _libCommonUtilsJs = __webpack_require__(159);

	var _libCommonUtilsJs2 = _interopRequireDefault(_libCommonUtilsJs);

	var _libCommonShapeJs = __webpack_require__(160);

	var _libCommonShapeJs2 = _interopRequireDefault(_libCommonShapeJs);

	var _libCommonRatingUtilsJs = __webpack_require__(161);

	var _libCommonRatingUtilsJs2 = _interopRequireDefault(_libCommonRatingUtilsJs);
	...
	var ReactRating = (function (_React$Component) {
		...
		function ReactRating(props) {
			...
			_get(Object.getPrototypeOf(ReactRating.prototype), 'constructor', this).call(this, props);
			this.state = {
				ready: false,
				destroy: false
			};
			this.settings = _libCommonUtilsJs2['default'].extend(_libConstantsRatingConstantsJs2['default'], this.props.settings);
		...
		_createClass(ReactRating, [{
			...
			value: function componentDidMount() {
				...
				for (var idx = 0; idx < this.settings.count; idx++) {
					document.getElementById(this.props.id).replaceChild(_react2['default'].findDOMNode(document.getElementById(this.props.id + '_grad' + idx)), _libCommonRatingUtilsJs2['default'].showNormalRating(_react2['default'].findDOMNode(document.getElementById(this.props.id + '_grad' + idx)), settings));
				}

				_libCommonRatingUtilsJs2['default'].showRating(document, this.props.id, settings);
			}
		}, {
			key: 'drawShape',
			value: function drawShape() {
				var shape = [];
				for (var idx = 0; idx < this.settings.count; idx++) {
					shape.push(_react2['default'].createElement(_libCommonShapeJs2['default'], { settings: this.settings, id: this.props.id, idx: idx,
						style: _libCommonRatingUtilsJs2['default'].formStyles(this.settings, idx) }));
				}
...
/***/ function(module, exports) {
	...
	exports["default"] = RatingConstants;
	module.exports = exports["default"];
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Utils = {
		...
		extend: function extend(defaultValues, optionValues) {
			for (var key in defaultValues) {
				if (optionValues !== undefined && optionValues.hasOwnProperty(key)) defaultValues[key] = optionValues[key];
			}return defaultValues;
		},
		colorToRGBA: function colorToRGBA(color) {
			var cvs = undefined,
			    ctx = undefined;
		...
		formulateNewColor: function formulateNewColor(settings, currentVal) {
			var avgFill = [];
			...
			for (var i = 0; i < 3; i++) {
				var diff = Math.round((startColorCoords[i] - endColorCoords[i]) / settings.count);
		...
		workOutPrecision: function workOutPrecision(precision, num) {
			var multiplactiveInverse = 1 / precision;
	...
	var Shape = (function (_React$Component) {
		...
		_createClass(Shape, [{
			...
			value: function handleMouseMove(event) {
				_libCommonRatingUtilsJs2['default'].onEnterOrClickEvent(document, this.props.id, this.props.settings, this.props.idx, event);
			}
		}, {
			key: 'handleMouseClick',
			value: function handleMouseClick(event) {
				_libCommonRatingUtilsJs2['default'].onEnterOrClickEvent(document, this.props.id, this.props.settings, this.props.idx, event, true);
			...
			value: function handleMouseLeave(event) {
				_libCommonRatingUtilsJs2['default'].showRating(document, this.props.id, this.props.settings, this.props.settings.rating);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var RatingUtils = {
		...
		showNormalRating: function showNormalRating(grads, settings) {
		...
		showRating: function showRating($$, propsId, settings) {
			var rating = arguments.length <= 3 || arguments[3] === undefined ? settings.rating : arguments[3];
			return (function () {

				if (settings.startColor) startColorCoords = _libCommonUtilsJs2['default'].colorToRGBA(settings.startColor);
				if (settings.endColor) endColorCoords = _libCommonUtilsJs2['default'].colorToRGBA(settings.endColor);

				for (var idx = 0; idx < settings.count; idx++) {
					var grads = $$.getElementById(propsId + '_grad' + idx);
					document.getElementById(propsId).replaceChild(_react2['default'].findDOMNode(document.getElementById(propsId + '_grad' + idx)), RatingUtils.showNormalRating(_react2['default'].findDOMNode(document.getElementById(propsId + '_grad' + idx)), settings));
				}
				...
				if (settings.reverse) {
					for (var idx = 0; idx < rating; idx++) {
						var j = settings.count - 1 - idx;
						var grad = $$.getElementById(propsId + '_grad' + j);
						...
						if (grad !== null) {
							...
							if (parseInt(rating) !== rating) {
								...
								var _grad = $$.getElementById(propsId + '_grad' + k);
								_grad.getElementsByTagName('stop')[0].setAttribute('offset', 100 - rating * 10 % 10 * 10 + '%');
								_grad.getElementsByTagName('stop')[0].setAttribute("stop-color", settings.backgroundColor);
								_grad.getElementsByTagName('stop')[1].setAttribute('offset', 100 - rating * 10 % 10 * 10 + '%');
								_grad.getElementsByTagName('stop')[1].setAttribute("stop-color", fillColor);
							}
							if (_libCommonUtilsJs2['default'].isDefined(endColorCoords)) {
								fillColor = _libCommonUtilsJs2['default'].formulateNewColor(settings, idx);
							}
						}
					}
				} else {
					...
					for (var idx = 0; idx < rating; idx++) {
						var grad = $$.getElementById(propsId + '_grad' + idx);

						if (grad !== null) {
							grad.getElementsByTagName('stop')[0].setAttribute('offset', '100%');
							grad.getElementsByTagName('stop')[0].setAttribute("stop-color", fillColor);
							if (rating * 10 % 10 > 0) {
								var _grad2 = $$.getElementById(propsId + '_grad' + (Math.ceil(rating) - 1));
								_grad2.getElementsByTagName('stop')[0].setAttribute('offset', rating * 10 % 10 * 10 + '%');
								_grad2.getElementsByTagName('stop')[0].setAttribute("stop-color", fillColor);
							}
							if (_libCommonUtilsJs2['default'].isDefined(endColorCoords)) {
								fillColor = _libCommonUtilsJs2['default'].formulateNewColor(settings, idx);
							}
		...
		formStyles: function formStyles(settings, idx) {
			var divStyles = [];
			var styles = settings.horizontal ? divStyles.push('display:inline-block;margin-right:' + settings.shapeGap + ';') : divStyles.push('display:block;margin-bottom:' + settings.shapeGap + ';');
			var swh = 'scaleX(' + (1 + settings.widthGrowth * idx) + ')';

			styles = settings.widthGrowth ? divStyles.push('transform: ' + swh + ';-webkit-transform: ' + swh + ';-moz-transform: ' + swh + ';-o-transform: ' + swh + ';-ms-transform: ' + swh + ';') : null;
			swh = 'scaleY(' + (1 + settings.heightGrowth * idx) + ')';
			styles = settings.heightGrowth ? divStyles.push('transform: ' + swh + ';-webkit-transform: ' + swh + ';-moz-transform: ' + swh + ';-o-transform: ' + swh + ';-ms-transform: ' + swh + ';') : null;

			return divStyles;
		},
		onEnterOrClickEvent: function onEnterOrClickEvent($$, propsId, settings, ith, event) {
			var update = arguments.length <= 5 || arguments[5] === undefined ? false : arguments[5];

			if (settings.readOnly) return;

			var svg = $$.getElementsByTagName('svg')[ith];
			var partial = 0;
			...
			var rating = ((settings.reverse ? settings.max - settings.min - ith : ith + 1) - partial) * count;
			rating = settings.min + Number(_libCommonUtilsJs2['default'].workOutPrecision(settings.precision, rating));
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _libReactJrateJs = __webpack_require__(163);

	var _libReactJrateJs2 = _interopRequireDefault(_libReactJrateJs);
	...
	setTimeout(function () {
		...
		_react2['default'].render(_react2['default'].createElement(_libReactJrateJs2['default'], { settings: newSettings, id: 'jrate' }), document.getElementById('jrate'));
	}, 2000);
	...
	var ReactRating = (function (_React$Component) {
		...
		_createClass(ReactRating, [{
			...
			value: function render() {
				...
				return _react2['default'].createElement('div', { style: styles }, this.drawShape());
			}
		}]);
